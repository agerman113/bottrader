"""
Bybit ФЬЮЧЕРСНЫЙ бот (Linear USDT Perpetual) — v4.2 FIXED
=========================================================
Исправление: убраны ссылки на несуществующие TRAILING_STEP_PCT/OFFSET_PCT.
Трейлинг полностью адаптивный (ATR) с минимальными значениями.
"""
...
SYMBOLS = [
    ...
    "AI16Z/USDT:USDT", "UNI/USDT:USDT",   "AAVE/USDT:USDT",
    "CRV/USDT:USDT",   "DYDX/USDT:USDT",  "JUP/USDT:USDT",
    "PENDLE/USDT:USDT","GMX/USDT:USDT",   "LDO/USDT:USDT",
    "ARB/USDT:USDT",   "OP/USDT:USDT",    "MATIC/USDT:USDT",
    "STX/USDT:USDT",   "IMX/USDT:USDT",   "STRK/USDT:USDT",
    "ZK/USDT:USDT",    "MANTA/USDT:USDT", "AXS/USDT:USDT",
    "SAND/USDT:USDT",  "MANA/USDT:USDT",  "GALA/USDT:USDT",
    "ENJ/USDT:USDT",   "ILV/USDT:USDT",   "PIXEL/USDT:USDT",
    "PORTAL/USDT:USDT","LINK/USDT:USDT",  "GRT/USDT:USDT",
    "FIL/USDT:USDT",   "ICP/USDT:USDT",   "RUNE/USDT:USDT",
    "INJ/USDT:USDT",   "SUI/USDT:USDT",   "APT/USDT:USDT",
    "SEI/USDT:USDT",   "TIA/USDT:USDT",   "PYTH/USDT:USDT",
    "JTO/USDT:USDT",   "W/USDT:USDT",     "ENA/USDT:USDT",
    "EIGEN/USDT:USDT", "HBAR/USDT:USDT",  "VET/USDT:USDT",
    "ALGO/USDT:USDT",  "IOTA/USDT:USDT",  "EOS/USDT:USDT",
    "XTZ/USDT:USDT",   "THETA/USDT:USDT", "FLOW/USDT:USDT",
    "KSM/USDT:USDT",   "CHZ/USDT:USDT",   "MASK/USDT:USDT",
    "1INCH/USDT:USDT", "COMP/USDT:USDT",  "ZRO/USDT:USDT",
    "NOT/USDT:USDT",   "HMSTR/USDT:USDT", "CATI/USDT:USDT",
]
...
# Адаптивный трейлинг (ATR)
TRAILING_ATR_PERIOD = 14
TRAILING_ATR_MULT   = 1.5
TRAILING_OFFSET_MULT= 1.0
MIN_TRAILING_STEP   = 0.25           # минимальный шаг в процентах
...
# ================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (СОХРАНЕНИЕ, ЗАГРУЗКА, ИСТОРИЯ) ==================
...
def получить_скор(symbol: str) -> dict:
    ...
    try:
        ...
        if sr["near_support"]:
            ...
            details["sr_signal"] = f"нейтрально (sup={sr['dist_to_sup_pct']:.2f}% res={sr['dist_to_res_pct']:.2f}%)"
...
def рассчитать_размер_позиции(score: int, баланс: float, atr_pct: float = 1.0) -> float:
    ...
    log.info(f"  📐 Скор={score} → риск={risk_pct:.1f}% (макс.убыток={max_loss_usdt:.2f} USDT) → маржа={margin_usdt:.2f} USDT")
...
def обновить_sl_на_бирже(symbol: str, new_sl: float, side: str = "long") -> bool:
    try:
        ...
        exchange.set_trading_stop(symbol, params={"category": "linear", "stopLoss": float(sl_str), "slTriggerBy": "MarkPrice", "positionIdx": 0})
        ...
        try:
            ...
            exchange.private_post_v5_position_trading_stop({"category": "linear", "symbol": coin_sym, "stopLoss": sl_str, "slTriggerBy": "MarkPrice", "positionIdx": "0"})
...
def мониторить_позицию(symbol: str, entry_price: float, qty: float, открыта_в: float, sl_цена: float, side: str = "long") -> str:
    ...
    # Адаптивные параметры на основе ATR
    try:
        ...
        if len(raw) >= 30:
            ...
            log.info(f"  📊 ATR = {atr_pct:.2f}% → шаг трейлинга {trailing_step*100:.2f}%, отступ {trailing_offset*100:.2f}%")
    except Exception as e:
        log.warning(f"  Не удалось рассчитать ATR, использую минимальные значения: {e}")

    фаза = 1
    текущий_sl = sl_цена
    пиковая_цена = entry_price
    следующий_трейл = entry_price * (1 + trailing_step) if side == "long" else entry_price * (1 - trailing_step)

    log.info(f"  🚦 Мониторинг | вход={entry_price:.8f} | безубыток @ {breakeven_price:.8f} | трейлинг шаг={trailing_step*100:.2f}% отступ={trailing_offset*100:.2f}% | активация трейлинга при +{MIN_PROFIT_FOR_TRAIL}%")
        ...
        try:
            ...
            if фаза == 1 and pnl_pct >= 0:
                ...
                if обновить_sl_на_бирже(symbol, new_sl, side):
                    ...
                    следующий_трейл = cur_price * (1 + trailing_step) if side == "long" else cur_price * (1 - trailing_step)
                    log.info(f"  🔒 БЕЗУБЫТОК! SL → {new_sl:.8f} (комиссии покрыты)")

            # Трейлинг после достижения минимальной прибыли
                    ...
                    if cur_price >= следующий_трейл:
                        ...
                        if new_sl > текущий_sl:
                            if обновить_sl_на_бирже(symbol, new_sl, side):
                                ...
                                log.info(f"  📈 ТРЕЙЛИНГ: пик={пиковая_цена:.8f} → SL={new_sl:.8f} (зафиксировано {(new_sl-entry_price)/entry_price*100:+.2f}%)")
                    ...
                    if cur_price <= следующий_трейл:
                        ...
                        if new_sl < текущий_sl:
                            if обновить_sl_на_бирже(symbol, new_sl, side):
                                текущий_sl = new_sl
                                следующий_трейл = cur_price * (1 - trailing_step)
                                log.info(f"  📈 ТРЕЙЛИНГ: пик={пиковая_цена:.8f} → SL={new_sl:.8f} (зафиксировано {(entry_price-new_sl)/entry_price*100:+.2f}%)")
                    if cur_price < пиковая_цена:
                        пиковая_цена = cur_price

            фаза_лейбл = {1: "обычная", 2: "безубыток 🔒", 3: "трейлинг 📈"}.get(фаза, "?")
            log.info(f"  [{coin}] {cur_price:.8f}  P&L={pnl_pct:+.2f}% ({pnl:+.4f}U)  SL={текущий_sl:.8f}  фаза={фаза_лейбл}  дед={до_дед}с")
        except Exception as e:
...
def аналитика_по_инструментам():
    ...
    for сд in история:
        ...
        if sym not in по_символам:
            по_символам[sym] = {"tp": 0, "sl": 0, "timeout": 0, "pnl": 0.0, "scores": [], "dur": []}
        r = сд.get("результат", "")
        if r == "tp":
    ...
    инд_ключи = ["rsi", "rsi_1h", "macd", "range_filter", "supertrend", "supertrend_15m", "hull", "тренд_1h", "тренд_15m", "тренд_4h", "adx", "stoch_k", "объём_ratio", "vwap_dev", "sr_signal"]
    индик_стат = {}
    for сд in история:
        det = сд.get("details", {})
        ...
        for к in инд_ключи:
            if к not in индик_стат:
                индик_стат[к] = {"bull_wins": 0, "bull_total": 0, "bear_wins": 0, "bear_total": 0}
            v = det.get(к)
            if v is None:
                continue
            бычий = False
            try:
                if к == "rsi":
                    ...
                    бычий = v == "вверх"
                elif к in ("supertrend", "supertrend_15m", "hull"):
                    бычий = v == "вверх"
                elif к in ("тренд_1h", "тренд_15m", "тренд_4h"):
                    бычий = v == "бычий"
                elif к == "adx":
                    бычий = float(v) > 25
                elif к == "stoch_k":
    ...
    log.info("  ─" * 35)
    log.info(f"  {'Символ':<16} {'Сделок':>6}  {'TP':>4}  {'SL':>4}  {'WR%':>6}  {'P&L':>8}  {'Ср.скор':>7}  {'Ср.мин':>7}")
    log.info("  " + "─" * 65)
    for sym, d in sorted(по_символам.items(), key=lambda x: x[1]["pnl"], reverse=True):
        ...
        log.info(f"  {sym:<16} {всего:>6}  {d['tp']:>4}  {d['sl']:>4}  {wr:>5.1f}%  {знак}{d['pnl']:>7.4f}U  {ср_sc:>6.1f}  {ср_мин:>6.1f}м")
    log.info("")
    log.info("  📈  ЭФФЕКТИВНОСТЬ ИНДИКАТОРОВ")
    log.info(f"  {'Индикатор':<18} {'🟢Бычий WR%':>11}  {'n':>4}  {'🔴Медвежий WR%':>14}  {'n':>4}  {'Разница':>8}")
    log.info("  " + "─" * 65)
    for инд, d in sorted(индик_стат.items(), key=lambda x: (x[1]["bull_wins"] / (x[1]["bull_total"] or 1)) - (x[1]["bear_wins"] / (x[1]["bear_total"] or 1)), reverse=True):
        ...
        log.info(f"  {инд:<18}  {b_wr:>9.1f}%  {d['bull_total']:>4}  {r_wr:>12.1f}%  {d['bear_total']:>4}  {знак}{diff:>+7.1f}%")
    log.info("")
    log.info("  💡 РЕКОМЕНДАЦИИ:")
    for инд, d in sorted(индик_стат.items(), key=lambda x: (x[1]["bull_wins"] / (x[1]["bull_total"] or 1)) - (x[1]["bear_wins"] / (x[1]["bear_total"] or 1)), reverse=True)[:3]:
        b_wr = d["bull_wins"] / d["bull_total"] * 100 if d["bull_total"] > 0 else 0
        r_wr = d["bear_wins"] / d["bear_total"] * 100 if d["bear_total"] > 0 else 0
        if b_wr - r_wr > 10 and d["bull_total"] >= 3:
            log.info(f"    ✅ {инд}: бычий сигнал даёт WR {b_wr:.1f}% — важный фильтр")
    for инд, d in list(sorted(индик_стат.items(), key=lambda x: (x[1]["bull_wins"] / (x[1]["bull_total"] or 1)) - (x[1]["bear_wins"] / (x[1]["bear_total"] or 1)), reverse=True))[-2:]:
        b_wr = d["bull_wins"] / d["bull_total"] * 100 if d["bull_total"] > 0 else 0
        r_wr = d["bear_wins"] / d["bear_total"] * 100 if d["bear_total"] > 0 else 0
    ...
    log.info("=" * 70)
    log.info("")
    отчёт = {"сформирован": datetime.now().strftime("%d.%m.%Y %H:%M:%S"), "всего_сделок": len(история), "по_символам": {k: {kk: vv for kk, vv in v.items() if kk not in ("scores","dur")} for k, v in по_символам.items()}, "эффективность_индикаторов": {к: {"bull_winrate_pct": round(d["bull_wins"]/d["bull_total"]*100, 1) if d["bull_total"] > 0 else None, "bull_n": d["bull_total"], "bear_winrate_pct": round(d["bear_wins"]/d["bear_total"]*100, 1) if d["bear_total"] > 0 else None, "bear_n": d["bear_total"]} for к, d in индик_стат.items()}}
    try:
...
def пост_трейд_анализ(запись: dict):
    ...
    log.info(f"  Результат: {знак} {r.upper()}   P&L: {pnl:+.4f} USDT   Длительность: {dur:.1f} мин")
...
def main():
    ...
    log.info("")
    log.info("=" * 60)
    log.info("  🤖  ФЬЮЧЕРСНЫЙ БОТ v4.2 (ADAPTIVE + BLOCK, FIXED)")
    log.info("")
    ...
    while True:
        try:
            ...
            if stats.get("sl_streak", 0) >= SL_STREAK_LIMIT:
                log.warning(f"  🧊 {SL_STREAK_LIMIT} SL подряд — cooldown {SL_STREAK_PAUSE//60} мин. Рынок сейчас против нас.")
                stats["sl_streak"] = 0
                сохранить_состояние()
            ...
            for sym in SYMBOLS:
                try:
                    ...
                    scores[sym] = res
                    sr = res.get("sr", {})
                    log.info(f"  {sym.split(':')[0]:12s}  скор={ai_score:3d}/100  rsi={res['details'].get('rsi', '?'):5}  rf={res['details'].get('range_filter', '?'):5}  st={res['details'].get('supertrend', '?'):5}  SR={res['details'].get('sr_signal', '?')}")
                except Exception as e:
                    log.warning(f"  Ошибка скора {sym}: {e}")
                    scores[sym] = {"score": 0, "score_final": 0, "details": {}, "price": 0, "sr": {}}

            кандидаты = sorted([(s, d) for s, d in scores.items() if d["score_final"] >= MIN_SCORE], key=lambda x: x[1]["score_final"], reverse=True)[:5]
            if not кандидаты:
                log.info(f"  Нет пар с скором >= {MIN_SCORE} — ждём {SCAN_INTERVAL} сек")
                time.sleep(SCAN_INTERVAL)
                continue

            log.info(f"  🏆 Топ кандидаты (лонг): " + ", ".join(f"{s.split(':')[0]}={d['score_final']}" for s, d in кандидаты))
            ...
            for лучшая, данные in кандидаты:
                ...
                if sr_info.get("near_resistance") and dist_res_now < SR_BLOCK_DIST_PCT:
                    log.info(f"  ⛔ {лучшая.split(':')[0]} (скор={фин_скор}): resistance в {dist_res_now:.3f}% — пробуем следующего")
                    continue
                ...
                if rsi_val > 68 and not sr_info.get("near_support"):
                    log.info(f"  ⚠️ {лучшая.split(':')[0]}: RSI перекуплен ({rsi_val:.1f}) без поддержки — пропускаем")
                    continue
                выбрана = лучшая
                log.info(f"  ► Выбрана {лучшая.split(':')[0]} (лонг)  скор={фин_скор}  цена={цена:.8f}  dist_res={dist_res_now:.3f}%")
                break
            ...
            if результат == "tp":
                ...
                log.warning(f"  ❌ SL: убыток ≈{pnl_сделки:.4f} USDT  (streak: {stats['sl_streak']}/{SL_STREAK_LIMIT}, символ заблокирован на {SYMBOL_BLOCK_MINUTES} мин)")
